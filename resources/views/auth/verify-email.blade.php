<x-guest-layout>
    <div class="auth-card-wrap">
        <div class="auth-page-intro">
            <span class="auth-page-chip auth-page-chip--green">{{ db_trans('email_verification') }}</span>
        ...
        <div class="auth-form-card">
            <div class="auth-info-box">{{ db_trans('check_your_inbox_and_click_the_verification_link_to_activate_your_account') }}</div>

            <div class="auth-form-actions auth-form-actions--stack">
                <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <button type="submit" class="auth-btn auth-btn--primary auth-btn--auto">{{ db_trans('resend_verification_email') }}</button>
                </form>
