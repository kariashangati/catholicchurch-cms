...
@section('title', db_trans('service_categories'))
...
<div class="admin-ui-v4 contributions-page-v4">
    ...
    <div class="ui-page-hero mb-4">
        ...
        <div class="row g-4 align-items-center position-relative">
            <div class="col-xl-8">
                <span class="ui-page-badge"><i class="fas fa-screwdriver-wrench"></i>{{ db_trans('service_categories') }}</span>
                <h1 class="ui-page-title mt-3 mb-2">{{ db_trans('service_categories') }}</h1>
            ...
            <div class="col-xl-4">
                <div class="ui-actions-grid">
                    @can('operations.service-categories.create')
                        <button type="button" class="ui-hero-action border-0 text-start" data-bs-toggle="modal" data-bs-target="#createServiceCategoryModal">
                            <span class="ui-hero-action-icon"><i class="fas fa-plus"></i></span>
                            <span class="ui-hero-action-text">{{ db_trans('add_service_category') }}</span>
                        </button>
                    @endcan
                    <a href="{{ Route::has('operations.service-providers.index') ? route('operations.service-providers.index') : '#' }}" class="ui-hero-action">
                        <span class="ui-hero-action-icon"><i class="fas fa-handshake"></i></span>
                        <span class="ui-hero-action-text">{{ db_trans('service_providers') }}</span>
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="ui-table-card p-4">
        <div class="ui-section-heading d-flex flex-wrap justify-content-between align-items-center gap-3">
            <div>
                <h5 class="mb-1">{{ db_trans('service_categories') }}</h5>
            </div>

            <div class="d-flex flex-wrap align-items-center gap-2">
                <span class="ui-section-badge">{{ number_format($categories->count()) }} {{ db_trans('records') }}</span>
                <a href="{{ route('pdf.operations.service-categories.export') }}" class="btn btn-sm btn-outline-danger">
                    <i class="fas fa-file-pdf me-1"></i>{{ db_trans('export_pdf') }}
                </a>
                <a href="{{ route('operations.service-categories.export.excel') }}" class="btn btn-sm btn-outline-success">
                    <i class="fas fa-file-excel me-1"></i>{{ db_trans('export_excel') }}
                </a>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table align-middle" id="serviceCategoriesTable">
                    ...
                    @foreach($categories as $category)
                        <tr>
                            ...
                            <td class="text-end text-nowrap">
                                @can('operations.service-categories.update')<button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editServiceCategoryModal{{ $category->id }}"><i class="fas fa-pen me-1"></i>{{ db_trans('edit') }}</button>@endcan
                                @can('operations.service-categories.delete')<form action="{{ route('operations.service-categories.destroy', $category) }}" method="POST" class="d-inline js-swal-delete">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-outline-danger"><i class="fas fa-trash me-1"></i>{{ db_trans('delete') }}</button></form>@endcan
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @foreach($categories as $category)
        @include('admin.operations.service-categories.partials.edit-modal', ['category' => $category])
    @endforeach
    @include('admin.operations.service-categories.partials.create-modal')
</div>
@endsection
