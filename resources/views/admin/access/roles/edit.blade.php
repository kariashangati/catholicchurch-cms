...
@section('title', db_trans('edit_role'))
@push('styles')
    ...
    <div class="d-flex flex-column gap-4">
        <div class="p-4 rounded-4 text-white" style="background:linear-gradient(135deg,#111827 0%,#1f2937 35%,#4338ca 100%);">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <div>
                    <h2 class="mb-2">{{ db_trans('edit_role') }}</h2>
                    <p class="mb-0 text-white-50">{{ $role->name }}</p>
                </div>
                <a href="{{ route('system-access.roles.index') }}" class="btn btn-light">{{ db_trans('back_to_roles') }}</a>
            </div>
        </div>

        <form method="POST" action="{{ route('system-access.roles.update', $role->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="text" name="guard_name" value="{{ old('guard_name', $role->guard_name) }}" class="form-control @error('guard_name') is-invalid @enderror" required>
                                ...
                                @foreach($items as $permission)
                                    <div class="col-lg-3 col-md-4 col-sm-6">
                                        <div class="form-check border rounded-3 p-3">
                                            ...
                                            <label class="form-check-label fw-semibold" for="perm_edit_{{ $permission->id }}">
                                                {{ $permission->name }}
                                            </label>
