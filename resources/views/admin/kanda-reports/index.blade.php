...
@section('title', db_trans('kanda_financial_reports'))
...
<div class="kanda-report-v3">
            ...
            <div class="col-lg-8">
                ...
                <div class="hero-meta-wrap">
                    <span class="hero-pill">
                        <i class="fas fa-compass me-2"></i>{{ $hero['scope_label'] }}
                    </span>
                    <span class="hero-pill">
                        <i class="fas fa-calendar-alt me-2"></i>{{ $hero['period'] }}
                    </span>
                    <span class="hero-pill hero-pill-warning">
                        <i class="fas fa-chart-line me-2"></i>{{ $hero['grand_total'] }}
                    </span>
                </div>
            </div>
    ...
    <div class="card filter-panel border-0 mb-4">
        <div class="card-body p-4">
            <form method="GET" action="{{ route('kanda-reports.index') }}">
            ...
            <div class="card analytics-panel border-0">
                <div class="card-body p-4">
                    <div class="panel-head">
                        <div>
                            <h5 class="panel-title">{{ db_trans('performance_by_kanda') }}</h5>
                        </div>
                    ...
                    <div class="chart-shell chart-shell-lg">
                        <canvas id="kandaReportFinanceChart"></canvas>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card dashboard-side-panel border-0 h-100">
                <div class="card-body p-4">
                    <div class="panel-head mb-4">
                        <div>
                            <h5 class="panel-title">{{ db_trans('income_mix') }}</h5>
                        </div>
                    </div>

                    <div class="chart-shell">
                        <canvas id="kandaReportCompositionChart"></canvas>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card table-panel border-0">
        <div class="card-header border-0 p-4">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                    <h5 class="panel-title mb-1">{{ db_trans('kanda_financial_reports') }}</h5>
                </div>

                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('pdf.kanda-reports.export', $exportQuery) }}" class="btn btn-sm btn-outline-danger btn-modern">
                        <i class="fas fa-file-pdf me-1"></i>{{ db_trans('export_pdf') }}
                    </a>

                    <a href="{{ route('kanda-reports.export.excel', $exportQuery) }}" class="btn btn-sm btn-outline-success btn-modern">
        ...
        <div class="card-body p-0">
            <div class="table-responsive dashboard-table-wrap">
                <table class="table align-middle table-hover mb-0 dashboard-table" id="kandaReportTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td>
                                    <div class="name-block">
                                        <div class="name-main">{{ $report->name }}</div>
                                        <div class="name-sub">{{ $report->code }}</div>
                                    </div>
                                </td>
                                <td>{{ number_format($report->jumuiyas_count) }}</td>
                                <td>{{ number_format($report->familias_count) }}</td>
                                <td>{{ number_format($report->members_count) }}</td>
                                <td>{{ $money($report->tithe_total) }}</td>
                                <td>{{ $money($report->offering_total) }}</td>

                                @foreach($contributionTypes as $type)
                                    <td>{{ $money(data_get($report, 'contribution_type_totals.' . $type->id, 0)) }}</td>
                                @endforeach

                                <td class="fw-bold text-success">{{ $money($report->grand_total) }}</td>
                                <td>
                                    <a href="{{ route('kanda-reports.show', ['kanda' => $report->id, 'year' => $selectedYear, 'month' => $selectedMonth]) }}" class="btn btn-sm btn-outline-primary btn-modern">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ 8 + $contributionTypes->count() }}" class="text-center py-5 text-muted">
                    ...
                    <tfoot>
                        <tr class="fw-bold">
                            ...
                            <td>{{ number_format($stats['total_jumuiyas']) }}</td>
                            <td>{{ number_format($stats['total_familias']) }}</td>
                            <td>{{ number_format($stats['total_members']) }}</td>
                            <td>{{ $money($stats['total_tithes']) }}</td>
                            <td>{{ $money($stats['total_offerings']) }}</td>

                            @foreach($contributionTypes as $type)
                                <td>{{ $money(data_get($stats, 'contribution_type_totals.' . $type->id, 0)) }}</td>
                            @endforeach

                            <td class="text-success">{{ $money($stats['grand_total']) }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
...
document.addEventListener('DOMContentLoaded', function () {
    if (window.jQuery && $.fn.DataTable && $('#kandaReportTable').length) {
        $('#kandaReportTable').DataTable({
            paging: true,
            info: true,
            searching: true,
            responsive: true,
