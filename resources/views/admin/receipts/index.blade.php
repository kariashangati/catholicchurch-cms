...
@php
    ...
    $selectedLayout = $filters['receipt_layout'] ?? 'mwanajumuiya';
@endphp
...
<div class="admin-ui-v4 contributions-page-v4">
        ...
        <div class="card-body p-4">
            <div class="row g-3 align-items-end">
                <div class="col-xl-2 col-md-4">
                    ...
                    <select name="receipt_type" id="receiptType" class="form-select">
                ...
                <div class="col-xl-2 col-md-4">
                    <label class="form-label">{{ db_trans('receipt_layout') }}</label>
                    <select name="receipt_layout" id="receiptLayout" class="form-select">
                        <option value="mwanajumuiya" @selected($selectedLayout === 'mwanajumuiya')>{{ db_trans('receipt_layout_member') }}</option>
                        <option value="jumuiya" @selected($selectedLayout === 'jumuiya')>{{ db_trans('receipt_layout_jumuiya') }}</option>
                        <option value="kanda" @selected($selectedLayout === 'kanda')>{{ db_trans('receipt_layout_kanda') }}</option>
                    </select>
                </div>
                ...
                <div class="col-xl-2 col-md-4 receipt-contribution-type-field">
    ...
    <form method="POST" action="{{ route('receipts.issue') }}" id="receiptIssueForm">
        @csrf
        @foreach(['receipt_type','receipt_layout','kanda_id','jumuiya_id','contribution_type_id','year','month'] as $field)
        ...
        <div class="ui-table-card p-4">
            ...
            <div class="alert alert-light border d-flex flex-wrap gap-2 align-items-center mb-3">
                ...
                @if($selectedLayout === 'kanda')
                    <span>{{ db_trans('receipt_kanda_summary_hint') }}</span>
                @elseif($selectedLayout === 'jumuiya')
                    <span>{{ db_trans('receipt_jumuiya_summary_hint') }}</span>
                @else
                    <span>{{ db_trans('receipt_member_selection_hint') }}</span>
                @endif
            </div>
            ...
            <div class="table-responsive">
                <table class="table align-middle">
                    ...
                    @empty
                        <tr><td colspan="9" class="text-center text-muted py-5">{{ db_trans('no_records_found') }}</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mt-3">
                ...
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    ...
                    <button type="submit" id="printSelectedReceipts" class="btn ui-btn-primary" disabled>
                        <i class="fas fa-print me-1"></i>{{ $selectedLayout === 'mwanajumuiya' ? db_trans('print_selected') : db_trans('print_summary_receipt') }}
                    </button>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const contributionTypeFields = document.querySelectorAll('.receipt-contribution-type-field');
    const form = document.getElementById('receiptIssueForm');
    const printButton = document.getElementById('printSelectedReceipts');
    ...
    const syncContributionType = function () {
        const isContribution = type && type.value === 'mchango';
        contributionTypeFields.forEach(function(field){ field.style.display = isContribution ? '' : 'none'; });
    };

    const refreshPrintButton = function () {
        if (!printButton) return;
        const selectedLayout = @json($selectedLayout);
        const hasPendingRows = document.querySelectorAll('.receipt-row-check').length > 0;
        const checked = document.querySelectorAll('.receipt-row-check:checked').length > 0;

        if (selectedLayout === 'mwanajumuiya') {
            printButton.disabled = !checked;
        } else {
            printButton.disabled = !hasPendingRows;
        }
    };
    ...
    if (type) type.addEventListener('change', syncContributionType);

    if (layout) {
        layout.addEventListener('change', function () {
            if (this.value === 'jumuiya' && jumuiya && !jumuiya.value) jumuiya.focus();
            if (this.value === 'kanda' && kanda && !kanda.value) kanda.focus();
        });
    }
    ...
    document.querySelectorAll('.receipt-row-check').forEach(function(el){
        el.addEventListener('change', refreshPrintButton);
    });
    ...
    if (form) {
        form.addEventListener('submit', function (event) {
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({ icon: 'warning', title: @json(db_trans('please_select_at_least_one_receipt')) });
            }
        });
    }
});
