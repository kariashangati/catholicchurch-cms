<div class="modal fade" id="{{ $modalId }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content"><form method="POST" action="{{ $action }}">@csrf @if(($method ?? 'POST') !== 'POST') @method($method) @endif
        ...
        <div class="modal-body"><div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">{{ db_trans('offering_date') }}</label><input type="date" name="offering_date" class="form-control" value="{{ old('offering_date', isset($item) ? optional($item->offering_date)->toDateString() : now()->toDateString()) }}" required></div>
            <div class="col-md-4"><label class="form-label">{{ db_trans('mass_type') }}</label><select name="mass_type_id" class="form-select"><option value="">—</option>@foreach($massTypes as $massType)<option value="{{ $massType->id }}" @selected(($item->mass_type_id ?? null) == $massType->id)>{{ $massType->name }}</option>@endforeach</select></div>
            <div class="col-md-6"><label class="form-label">Mass Name</label><input type="text" name="mass_name" class="form-control" value="{{ old('mass_name', $item->mass_name ?? '') }}"></div>
