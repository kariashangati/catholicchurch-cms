            ...
            <form method="POST" action="{{ $action }}">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><label class="form-label">{{ db_trans('jumuiyas') }}</label><select name="jumuiya_id" class="form-select"><option value="">—</option>@foreach($jumuiyas as $jumuiya)<option value="{{ $jumuiya->id }}" @selected(($item->jumuiya_id ?? null) == $jumuiya->id)>{{ $jumuiya->name }}</option>@endforeach</select></div>
                        <div class="col-md-6"><label class="form-label">{{ db_trans('amount') }}</label><input type="number" step="0.01" min="0.01" name="amount" class="form-control" value="{{ old('amount', $item->amount ?? '') }}" required></div>
                        <div class="col-md-6"><label class="form-label">{{ db_trans('contribution_date') }}</label><input type="date" name="contribution_date" class="form-control" value="{{ old('contribution_date', isset($item) ? optional($item->contribution_date)->toDateString() : now()->toDateString()) }}" required></div>
