...
@section('content')
    @php
        ...
        $types = collect($types ?? []);
        $statuses = $statuses ?? \App\Models\CashContribution::availableStatuses();
        $filters = $filters ?? [];

        $currency = fn ($amount) => number_format((float) $amount, 2);

        $totalAmount = (float) $items->sum('amount');
        $approvedCount = $items->where('status', \App\Models\CashContribution::STATUS_APPROVED)->count();
        $pendingCount = $items->where('status', \App\Models\CashContribution::STATUS_PENDING)->count();
        $rejectedCount = $items->where('status', \App\Models\CashContribution::STATUS_REJECTED)->count();
    ...
    <div class="admin-ui-v4 contributions-page-v4">
        <div class="ui-page-hero mb-4">
            ...
            <div class="row g-4 align-items-center position-relative">
                <div class="col-xl-8">
                    ...
                    <h1 class="ui-page-title mt-3 mb-2">{{ db_trans('cash_contributions') }}</h1>
                    ...
                    <div class="ui-meta-wrap mt-3">
                        ...
                        <span class="ui-meta-pill">
                            ...
                            {{ number_format($members->count()) }} {{ db_trans('members') }}
                        </span>
                ...
                <div class="col-xl-4">
                    <div class="ui-actions-grid">
                        @can('finance.contributions.cash.create')
                            <button type="button" class="ui-hero-action border-0 text-start" data-bs-toggle="modal" data-bs-target="#createCashModal">
                                <span class="ui-hero-action-icon"><i class="fas fa-plus"></i></span>
                                <span class="ui-hero-action-text">{{ db_trans('new_cash_contribution') }}</span>
                            </button>
                        @endcan
        ...
        <div class="card ui-filter-card border-0 mb-4">
            <div class="card-body p-4">
                <form method="GET" action="{{ route('finance.contributions.cash.index') }}">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-xl-2 col-md-6 d-flex gap-2">
                            ...
                            <a href="{{ route('finance.contributions.cash.index') }}" class="btn ui-btn-light w-100">
                                {{ db_trans('reset') }}
                            </a>
                        </div>
                    </div>
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-xl-3 col-md-6">
                <div class="ui-mini-card p-4 h-100">
                    ...
                    <div class="ui-stat-label">{{ db_trans('imeidhinishwa') }}</div>
                    <div class="ui-stat-value">{{ number_format($approvedCount) }}</div>
                </div>
            </div>
    ...
    <div>
        <h5 class="mb-1">{{ db_trans('cash_contributions') }}</h5>
    </div>

    <div class="d-flex flex-wrap gap-2 align-items-center">
        <span class="ui-section-badge">{{ number_format($items->count()) }} {{ db_trans('records') }}</span>

        @if(Route::has('pdf.finance.contributions.cash.export'))
            <a href="{{ route('pdf.finance.contributions.cash.export', request()->query()) }}" target="_blank" class="btn btn-sm btn-danger rounded-pill px-3">
                <i class="fas fa-file-pdf me-1"></i>{{ db_trans('export_pdf') }}
            </a>
        @endif

        @if(Route::has('finance.contributions.cash.export.excel'))
            <a href="{{ route('finance.contributions.cash.export.excel', request()->query()) }}" class="btn btn-sm btn-success rounded-pill px-3">
        ...
        @endif
            ...
            @if($items->isNotEmpty())
                <div class="table-responsive">
                    <table class="table align-middle" id="cashContributionTable">
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="ui-status-pill {{ $statusClass($item->status ?? \App\Models\CashContribution::STATUS_PENDING) }}">
                                            {{ db_trans($item->status ?? \App\Models\CashContribution::STATUS_PENDING) }}
                                        </span>
                                    </td>

                                    <td class="text-nowrap">
                                        @can('finance.contributions.cash.update')
                                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editCashModal{{ $item->id }}">
                                                <i class="fas fa-pen me-1"></i>{{ db_trans('edit') }}
                                            </button>
                                        @endcan

                                        @can('finance.contributions.cash.delete')
                                            <form method="POST" action="{{ route('finance.contributions.cash.destroy', $item) }}" class="d-inline js-swal-delete">
                                                @csrf
                                                @method('DELETE')
        ...
        @foreach($items as $item)
            @include('admin.finance.contributions.partials.cash-modal', [
                'modalId' => 'editCashModal' . $item->id,
                'title' => db_trans('edit') . ' ' . db_trans('cash_contributions'),
                'action' => route('finance.contributions.cash.update', $item),
                'method' => 'PUT',
                'item' => $item,
                'members' => $members,
                'types' => $types,
    ...
    @include('admin.finance.contributions.partials.cash-modal', [
        'modalId' => 'createCashModal',
        'title' => db_trans('new_cash_contribution'),
        'action' => route('finance.contributions.cash.store'),
        'method' => 'POST',
        'item' => null,
        'members' => $members,
        'types' => $types,
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            if (window.jQuery && $.fn.DataTable && $('#cashContributionTable').length) {
                $('#cashContributionTable').DataTable({
                    paging: true,
                    info: true,
                    searching: true,
                    responsive: true,
