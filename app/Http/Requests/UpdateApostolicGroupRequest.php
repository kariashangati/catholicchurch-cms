...
class UpdateApostolicGroupRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can('apostolic-groups.update') ?? false;
    }

    public function rules(): array
    {
        $group = $this->route('apostolicGroup');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique('apostolic_groups', 'name')->ignore($group->id)],
            'code' => ['required', 'string', 'max:50', Rule::unique('apostolic_groups', 'code')->ignore($group->id)],
