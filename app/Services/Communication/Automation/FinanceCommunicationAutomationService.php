...
class FinanceCommunicationAutomationService
{
    ...
    public function handleEvent(string $eventKey, mixed $record): ?CommunicationCampaign
    {
        ...
        $messageBody = method_exists($this->templateRenderer, 'renderBodyString')
            ? $this->templateRenderer->renderBodyString($template->body, $payload)
            : data_get($this->templateRenderer->render($template, $payload), 'rendered_body', $template->body);
        ...
        return DB::transaction(function () use ($automation, $template, $member, $record, $eventKey, $messageBody, $segmentData, $phone) {
            ...
            CommunicationMessage::create([
                ...
                'jumuiya_id' => $member->familia?->jumuiya_id ?? null,
                'kanda_id' => $member->familia?->jumuiya?->kanda_id ?? null,
    ...
    protected function resolvePhone(Member $member): ?string
    {
        ...
        return method_exists($this->phoneNumberService, 'normalizeTanzanianNumber') ? $this->phoneNumberService->normalizeTanzanianNumber($raw) : $this->phoneNumberService->normalize($raw);
    }
